...
const job= data 
  ...
  <div className="max-w-[980px] mx-auto mt-4">
    ...
    {data?.postDate && (
      <p className="text-[14px] text-red-600 mb-2">
        Post Date/Update: <span className="text-black">{data.postDate}</span>
      </p>
    )}
    {data?.mdescription && (
      <p className="text-[16px] text-red-600">
        Information: <span className="text-black">{data.mdescription}</span>
      </p>
    )}
  </div>
  ...
  <div className="max-w-[980px] m-4 mx-auto border grid grid-row-5">
    ...
    {(data?.postDate || data?.lastDate || data?.correctionDate || data?.adminCardDate) && (
    ...
    {(data?.applicationfeesG_O_EWs || data?.applicationfees_SC_ST) && (
      <div className="text-black grid grid-cols-1 md:grid-cols-2">
...
{data?.status !== "Active"?( <p className="text-red-600 text-lg font-semibold text-center">Job has been Expired</p>):( <div>
     {data?.applylink && (
    ...
    {data?.officialwebsitelink && (
      ...
      <div className="max-w-[980px] mx-auto mt-4 lg:mt-6">
        <div className="bg-violet-700 py-6 px-4 rounded-lg shadow-lg">
          <h1 className="text-center text-2xl lg:text-3xl text-white font-semibold">Job Details</h1>
          <p className="text-center text-3xl lg:text-4xl text-white mt-4 font-bold">{data.postName}</p>
          {data.location && (
            <p className="text-center text-xl text-white mt-2">{data.location}</p>
          )}
      ...
      <div className="max-w-[980px] mx-auto bg-gray-100 rounded-lg shadow-lg px-6 py-8">
        {data.requirementdata?.length > 0 && (
          <>
            <h2 className="text-xl font-semibold mb-2 text-violet-700">Requirements:</h2>
        ...
        {data.jobDescription?.length > 0 && (
          <>
            <h2 className="text-xl font-semibold mb-2 text-violet-700">Job Description:</h2>
            <ul className="space-y-2 list-disc list-inside text-gray-800 mb-6">
        ...
        {data.skill?.length > 0 && (
          <>
            <h2 className="text-xl font-semibold mb-2 text-violet-700">Skills Required:</h2>
            <ul className="space-y-2 list-disc list-inside text-gray-800 mb-6">
  ...
  {data?.status !== "Active" ? (
    <p className="text-red-600 text-lg font-semibold">Job has been Expired</p>
  ) : (
    <a
      href={data.applylink}
      target="_blank"
